...
class TestEstimatorQNN(QiskitMachineLearningTestCase):
    ...
    def test_estimator_qnn_1_1(self):
        ...
        estimator = Estimator()
        g = ParamShiftEstimatorGradient(estimator)
        estimator_qnn = EstimatorQNN(estimator, qc, [op], [params[0]], [params[1]], gradient=g)
        weights = np.array([1])
        ...
        for i, inputs in enumerate(test_data):
            ...
            print(f"res = {res}")
            np.testing.assert_allclose(res, correct_results[i], atol=1e-3)
