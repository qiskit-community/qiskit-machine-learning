...
class TestQBayesianInferenceV2(QiskitMachineLearningTestCase):
    """Test QBayesianInference Algorithm V2"""
    ...
    def setUp(self):
        ...
        # Quantum Bayesian inference
        qc = self._create_bayes_net()
        self.qbayesian = QBayesian(qc, sampler=self._sampler)
    ...
    def test_inference(self):
        ...
        res.append(self.qbayesian.inference(query=test_q_4, evidence=test_e_4))
        # Correct inference
        np.testing.assert_allclose(true_res, res, atol=0.04)
        # No change in samples
        self.assertTrue(samples[0] == samples[1])
